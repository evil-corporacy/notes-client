...
const NoteEdit = ({handleChangeImage, handleSaveColors, colors, pasteInNote}: {handleChangeImage: (src: string | ArrayBuffer | null) => void, handleSaveColors: (newColors: string[]) => void, colors: string[], pasteInNote: (message: any) => void}) => {
    ...
    return (
        <>
            <PaletteModal handleSaveColors={handleSaveColors} open={paletteOpen} setOpen={setPaletteOpen} colors={colors}/>
            <AiModal pasteInNote={paste} open={aiOpen} setOpen={setAiOpen}/>
            <ChangePictureModal handleSaveColors={handleSaveColors} handleChangeImage={handleChangeImage} open={pictureOpen} setOpen={setPictureOpen} colors={colors}/>
            <div className="flex gap-2 py-2">
                <IconButton variant="solid" onClick={() => setPictureOpen(true)} sx={{background: colors[0], color: colors[2], width: 32, height: 32}}>
                    <Panorama/>
                </IconButton>
